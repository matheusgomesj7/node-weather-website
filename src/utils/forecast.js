...
const forecast = (lat, lon, callback) => {
  ...
  request({ url, json: true }, (err, { body: { error, current } }) => {
    if (err) {
      callback("Unable to connect to weather service!")
    } else if (error) {
      callback(`There was an error of type ${error.type}, and of code ${error.code}`)
    } else {
      callback(null, `${current.weather_descriptions}. It is currently ${current.temperature} outside and it feels like ${current.feelslike} outside.`)
    }
  })
}
